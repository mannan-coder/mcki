          ...
          <table className="w-full">
            <thead className={`${isDarkMode ? 'bg-gray-800/90' : 'bg-gray-50/90'}`}>
              <tr className={`border-b ${isDarkMode ? 'border-gray-700/40' : 'border-gray-200/40'}`}>
                ...
                <th className={`px-2 py-2 text-center text-xs font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    ...
                    {allPrices.map((priceData, idx) => (
                      <td key={idx} className="px-2 py-3 text-center">
                        <div className="space-y-1">
                          <div className={`text-sm font-bold transition-colors ${getPriceColor(priceData.price)}`}>
